...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const typedUser = user as { photoURL?: string }  // Type assertion to fix photoURL error
  ...
  const userInitials = profile?.name
    ? profile.name
        .split(" ")
        .map((n: string) => n[0]) // explicit typing fix
        .join("")
        .toUpperCase()
    : "U"
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:pl-64">
        ...
        <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
          {/* Removed extra SheetTrigger here, handled above inside <Sheet> */}
